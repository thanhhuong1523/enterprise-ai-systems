...
*   **Anh Vũ Hoàng Nam (Quản trị viên Kỹ thuật - SYSTEM_ADMIN)**: Cần quản trị tài khoản và phân quyền người dùng (không thuộc bất kỳ phòng ban nghiệp vụ nào và bị chặn truy cập tất cả API tài liệu), xem nhật ký thao tác (Audit Log) để phát hiện bất thường, nhưng không được phép đọc nội dung chi tiết hoặc siêu dữ liệu của tài liệu để tránh lộ thông tin nội bộ.
...
| **NFR-03** | **Bảo mật quản trị (Security)** | **Must Have** | Người quản trị hệ thống chỉ được cấu hình nhân sự và xem nhật ký thao tác nghiệp vụ, không có quyền mở/đọc nội dung của bất kỳ tài liệu nào. |
...
*   **BR-02**: Tài liệu thuộc Ban Giám đốc (BOARD) là tài liệu tuyệt mật, chỉ thành viên BOARD mới được phép tải lên và xem nội bộ. Không một ai khác (bao gồm cả Admin và nhân viên phòng ban khác) có quyền truy cập hoặc đọc tài liệu của phòng BOARD. Phòng BOARD chỉ sử dụng tài liệu nội bộ của mình, hoàn toàn cấm tạo Alias chia sẻ ra ngoài và cấm nhận Alias từ phòng ban khác.
...
*   **BR-06**: Người quản trị hệ thống (`SYSTEM_ADMIN`) bị tước quyền xem trực tiếp nội dung hoặc siêu dữ liệu tài liệu (bị chặn truy cập toàn bộ các API `/api/v1/original-documents` và `/api/v1/alias-documents`). Tài khoản Admin không thuộc bất kỳ phòng ban nghiệp vụ nào. Nghiêm cấm tài khoản Admin tự thay đổi phòng ban hoặc vai trò của chính mình để tránh leo thang đặc quyền.
...
*   **BR-12 (Ghi nhật ký Audit)**: Nhật ký Audit là bất biến và chỉ hỗ trợ ghi mới (INSERT) và đọc (SELECT). Cấm mọi API chỉnh sửa hay xóa Audit log. Hệ thống phải tự động ghi nhận nhật ký nghiệp vụ cho tất cả các thao tác thay đổi phòng ban, vai trò của người dùng.
...
*   **BR-14 (Quyền thu hồi Alias)**: Chỉ phòng ban tạo Alias (phòng sở hữu tài liệu gốc) mới có quyền thu hồi/xóa mềm các Alias do họ tạo ra để chia sẻ cho phòng ban khác. Phòng ban nhận Alias tuyệt đối không có quyền tự ý xóa Alias này.
