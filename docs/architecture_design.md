...
### 4.1. Cô lập tự động qua Hibernate Filter kết hợp (Secure by Default - Không sử dụng Subquery)
Mọi truy vấn dữ liệu từ ứng dụng lên bảng gộp `documents` đều bị lọc tự động bởi Hibernate Filter được cấu hình trên thực thể `Document` để đảm bảo cô lập tuyệt đối mà không làm đứt gãy luồng chia sẻ và thu hồi Alias, đồng thời tối ưu hóa hiệu năng tối đa bằng cách loại bỏ hoàn toàn các câu truy vấn con lồng nhau (subqueries).
Hệ thống sử dụng các trường phi bình thường hóa `creator_department_id` (lưu phòng ban sở hữu tài liệu gốc đối với Alias) và `shared_department_ids` (mảng `uuid[]` trên tài liệu gốc lưu danh sách các phòng ban được chia sẻ). Bộ lọc áp dụng điều kiện kết hợp không dùng subquery:
```sql
...
```
- Đối với tài liệu gốc (`parent_id IS NULL`): hiển thị nếu phòng ban hiện tại sở hữu nó (`owner_department_id = :userDeptId`) hoặc được chia sẻ trong danh sách (`:userDeptId = ANY(shared_department_ids)`). Điều này cho phép phòng nhận Alias truy vấn Join/Select đến tài liệu gốc chéo phòng ban một cách trực tiếp và nhanh chóng (hỗ trợ bởi GIN index).
- Đối với tài liệu liên kết (`parent_id IS NOT NULL`): hiển thị nếu phòng ban hiện tại là phòng nhận Alias (`owner_department_id = :userDeptId`) hoặc là phòng tạo Alias (`creator_department_id = :userDeptId`). Điều này đảm bảo cả phòng nhận và phòng gửi đều nhìn thấy và quản lý được bản ghi Alias mà không cần thực hiện JOIN phức tạp dưới DB.
...
*   `documents`: Bảng gộp lưu trữ cả tài liệu gốc và Alias. Có các trường phi bình thường hóa `creator_department_id` (lưu phòng ban sở hữu tài liệu gốc đối với Alias) và `shared_department_ids` (mảng `uuid[]` trên tài liệu gốc lưu danh sách các phòng ban được chia sẻ). Có ràng buộc `CHECK` ở tầng CSDL để đảm bảo tính toàn vẹn giữa UUID LSB và loại tài liệu (nếu `parent_id` là NULL thì bit cuối của UUID phải là 0; nếu `parent_id` NOT NULL thì bit cuối UUID phải là 1). Có GIN Index trên `shared_department_ids` để tối ưu hóa hiệu năng Hibernate Filter. Có Unique Index có điều kiện trên `(parent_id, owner_department_id) WHERE deleted_at IS NULL` để chống trùng lặp Alias.
*   `audit_logs`: Nhật ký bất biến, áp dụng Hibernate Filter cô lập theo phòng ban (`actor_department = :userDeptId`) cho người dùng thường, chỉ cho phép quản trị viên hệ thống (`SYSTEM_ADMIN`) xem toàn bộ log.
