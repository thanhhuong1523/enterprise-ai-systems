...
public class DocumentDeduplicationHelperImpl implements DocumentDeduplicationHelper {
    ...
    private static final String SQL_AGGREGATE_CHECK =
            "SELECT " +
            "    bool_or(owner_department_id = CAST(? AS uuid) AND deleted_at IS NULL) AS has_active_in_dept, " +
            "    (array_agg(id ORDER BY created_at ASC) FILTER (WHERE owner_department_id = CAST(? AS uuid) AND deleted_at IS NULL))[1] AS active_doc_id, " +
    ...
    public DeduplicationQueryResult executeAggregateCheck(String hash, UUID departmentId) {
        log.debug("Executing aggregate check: hash={}, dept={}", hash, departmentId);
        ...
        }, departmentId, departmentId, hash);
    }
}
